...
class MedusaWP_Demo_Dark_Mode {
	...
	public function __construct() {
		...
		// Add HTML classes.
		add_filter( 'medusawp_demo_html_classes', array( $this, 'html_classes' ) );
	...
	public function editor_custom_color_variables() {
		...
		if ( $should_respect_color_scheme && MedusaWP_Demo_Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
		...
		wp_enqueue_script(
			'medusawp-demo-dark-mode-support-toggle',
		...
		wp_enqueue_script(
			'medusawp-demo-editor-dark-mode-support',
			get_template_directory_uri() . '/assets/js/editor-dark-mode-support.js',
			array( 'medusawp-demo-dark-mode-support-toggle' ),
			'1.0.0',
			true
		);
	}
	...
	public function customize_controls_enqueue_scripts() {
		...
		wp_enqueue_script(
			'medusawp-demo-customize-controls',
			get_template_directory_uri() . '/assets/js/customize.js',
			array( 'customize-base', 'customize-controls', 'underscore', 'jquery', 'medusawp-demo-customize-helpers' ),
			'1.0.0',
			true
		);
	}
	...
	public function customizer_controls( $wp_customize ) {
		...
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'Colors & Dark Mode', 'medusawp-demo' );
		}
		...
		$wp_customize->add_control(
			new MedusaWP_Demo_Customize_Notice_Control(
				$wp_customize,
				'respect_user_color_preference_notice',
				array(
					'section'         => 'colors',
					...
					'active_callback' => static function () {
						return 127 >= MedusaWP_Demo_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) );
					},
				)
			)
		);
		...
		$description  = '<p>';
		$description .= sprintf(
			/* translators: %s: MedusaWP Demo support article URL. */
			__( 'Dark Mode is a device setting. If a visitor to your site requests it, your site will be shown with a dark background and light text. <a href="%s">Learn more about Dark Mode.</a>', 'medusawp-demo' ),
			esc_url( __( 'https://wordpress.org/documentation/article/twenty-twenty-one/#dark-mode-support', 'medusawp-demo' ) )
		);
		$description .= '</p>';
		$description .= '<p>' . __( 'Dark Mode can also be turned on and off with a button that you can find in the bottom corner of the page.', 'medusawp-demo' ) . '</p>';
		...
		$wp_customize->add_control(
			...
			array(
				'type'            => 'checkbox',
				'section'         => 'colors',
				'label'           => esc_html__( 'Dark Mode support', 'medusawp-demo' ),
				'priority'        => 110,
	...
	public function html_classes( $classes ) {
		...
		$background_color            = get_theme_mod( 'background_color', 'D1E4DD' );
		$should_respect_color_scheme = get_theme_mod( 'respect_user_color_preference', false );
		if ( $should_respect_color_scheme && 127 <= MedusaWP_Demo_Custom_Colors::get_relative_luminance_from_hex( $background_color ) ) {
	...
	public function admin_body_classes( $classes ) {
		...
		if ( $current_screen->is_block_editor() ) {
			...
			if ( $should_respect_color_scheme && MedusaWP_Demo_Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
				$classes .= ' medusawp-demo-supports-dark-theme';
			}
		}
	...
	public function switch_should_render() {
		...
		return (
			...
			! $is_IE &&
			127 <= MedusaWP_Demo_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) )
		);
	}
	...
	public function the_html( $attrs = array() ) {
		...
		printf(
			...
			esc_html__( 'Dark Mode: %s', 'medusawp-demo' ),
		...
		<style>
			...
			#dark-mode-toggler > span::before {
				content: '<?php esc_attr_e( 'Off', 'medusawp-demo' ); ?>';
			}
			#dark-mode-toggler[aria-pressed="true"] > span::before {
				content: '<?php esc_attr_e( 'On', 'medusawp-demo' ); ?>';
			}
	...
	public function add_privacy_policy_content() {
		...
		$content = '<p class="privacy-policy-tutorial">' . __( 'MedusaWP Demo uses LocalStorage when Dark Mode support is enabled.', 'medusawp-demo' ) . '</p>'
				. '<strong class="privacy-policy-tutorial">' . __( 'Suggested text:', 'medusawp-demo' ) . '</strong> '
				. __( 'This website uses LocalStorage to save the setting when Dark Mode support is turned on or off.<br> LocalStorage is necessary for the setting to work and is only used when a user clicks on the Dark Mode button.<br> No data is saved in the database or transferred.', 'medusawp-demo' );
		wp_add_privacy_policy_content( __( 'MedusaWP Demo', 'medusawp-demo' ), wp_kses_post( wpautop( $content, false ) ) );
	}
}
